                ...
                <div className='flex justify-between items-center gap-10 w-full'>
                    <div className={`p-4 ${isLight ? "border-black" : "border-white"} w-full md:w-1/2`}>
                        {projects.map((project, index) => (
                            <div key={index} className={`p-4 rounded-xl border-2 ${isLight ? "border-black" : "border-white"} flex flex-col items-start justify-center cursor-pointer`} onClick={() => setActive(index)}>
                                <div className='flex justify-between items-center w-full'>
                                    <h3 className={`${styles.sectionSubText} ${isLight ? "text-black-200" : "text-white-100" }`}>{project.title}</h3>
                                    <img src={dropdown} alt="dropdown" className={`w-10 h-10 ${isLight ? "" : "invert"} ${active === index ? "hidden" : ""}`} />
                                </div>
